...
- New public method `copyRaw(host_addr, CompositeAddress*, to_device)` wrapping the existing straight-byte-copy path (DMA params with a **null** conversion descriptor) and the existing H2D/D2H launch.
- Copy length is the device allocation's physical size (`total_size()` — the padded/tiled byte count, **not** `numel × itemsize`).
- Single-chunk is the common case; the multi-chunk (1p5) path is **M2-F3** (__M2F3__), not here.
