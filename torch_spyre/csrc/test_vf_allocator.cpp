 ...
 *   g++ -std=c++17 -DTEST_VF_ALLOCATOR test_vf_allocator.cpp -o
 * test_vf_allocator
 *   ./test_vf_allocator
 *
...
class TestFramework {
 public:
  ...
  struct Test {
    std::string name;  // NOLINT(build/include_what_you_use)
    void (*func)();
  };
...
TEST(SegmentInfoCreation) {
  ...
  std::vector<SegmentInfo> segments;  // NOLINT(build/include_what_you_use)
  for (int i = 0; i < 8; i++) {
...
TEST(FreeIntervalMerging) {
  ...
  std::set<FreeInterval> free_intervals;  // NOLINT(build/include_what_you_use)
...
int main() {
  // NOLINT comments below suppress false cpplint warnings
